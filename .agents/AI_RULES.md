...
- Setiap kali user meminta pembuatan, revisi, atau redesign UI, **wajib cek daftar komponen shadcn yang tersedia terlebih dahulu** sebelum mulai implementasi:
    - Gunakan `npx shadcn@latest list '@shadcn'` untuk melihat daftar komponen/blok dari registry shadcn.
    - Gunakan `npx shadcn@latest search '@shadcn' -q "<kebutuhan-komponen>"` untuk mencari komponen yang relevan.
    - Catatan PowerShell: registry `@shadcn` harus ditulis dengan quote (`'@shadcn'`) agar tidak dibaca sebagai syntax PowerShell.
