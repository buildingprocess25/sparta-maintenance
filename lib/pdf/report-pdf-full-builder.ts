...
import {
    generateReportPdfFull,
    type ChecklistItemWithPhotos,
...
/**
 * Extracts checklist photo URLs from each item's `images` or `photoUrl` field.
 * Returns items that have at least one photo URL.
 */
...
export async function buildReportPdfFullBuffer(reportNumber: string): Promise<{
    ...
    const completionAdditionalPhotos: string[] = Array.isArray(rawAdditionalDocs)
    ...
    // Extract checklist photos for documentation pages
    const checklistPhotos = extractChecklistPhotos(items);

    const buffer = await generateReportPdfFull(
        {
