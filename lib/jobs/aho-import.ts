...
export type AhoImportResult = {
    ...
    skipped: number;
    total: number;
    printDate: string | null; // ISO string agar bisa disimpan ke JSON
    errors: string[];
    duplicates: string[];
};
...
// ─── UPSERT Helper (dipindahkan dari actions.ts) ──────────────────────────────
...
async function _runImportJob(
    ...
    try {
        ...
        // Ambil semua tiket existing di DB
        ...
        // Klasifikasi Create / Update / Delete
        ...
        // Delete tiket yang tidak ada di file
        let deleted = 0;
        ...
        // Simpan printDate ke AppSetting
        if (printDate) {
        ...
        // Update job ke done; bersihkan fileBuffer untuk hemat storage
        await prisma.ahoImportJob.update({
            where: { id: jobId },
            data: {
                status: "done",
