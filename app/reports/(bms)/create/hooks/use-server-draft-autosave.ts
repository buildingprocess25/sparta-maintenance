...
type ServerDraftFlushReason = "store" | "photo" | "step" | "idle" | "pagehide" | "submit";
...
export function useServerDraftAutosave({ selectedStoreCode, isSubmitting, buildDraftData, setDraftReportId }: UseServerDraftAutosaveParams) {
  const dirtyRef = useRef(false);
  const dirtyVersionRef = useRef(0);
  const inFlightRef = useRef(false);
  const inFlightPromiseRef = useRef<Promise<ServerDraftFlushResult> | null>(null);
