...
import {
  ...
  type LocalPhoto,
  type StartWorkMaterialStoreEntry,
} from "./types";
...
type CameraTarget =
  ...
  | { target: "additional" }
  | { target: "startSelfie" | "startReceipt" }
  | { target: "startStore"; storeId: string }
  | null;
...
export function useCompletionWorkForm(report: CompletionReport, maxAvailableBudget: number) {
  ...
  const shouldReviseStartWork = report.status === "REVIEW_REJECTED_REVISION";
  const isZeroCost = totalEstimation === 0;

  const [isRestoringDraft, setIsRestoringDraft] = useState(true);
  const [itemStates, setItemStates] = useState(() => buildItemStates(report));
  ...
  >(() =>
    report.startMaterialStores.map((store, idx) => ({
      ...
      name: store.name,
      city: store.city,
      photos: (store.photoUrls || []).map((url, i) => toRemotePhoto(url, i)),
    })),
  );
  ...
  const grandTotal = useMemo(
    () =>
      damagedItems.reduce((sum, item) => {
        ...
        if (!state) return sum;
        return (
          sum + realisasiNetTotal(state.realisasiEntries, state.discountAmount)
        );
      }, 0),
    [damagedItems, itemStates],
  );
  ...
  const buildDraftData = useCallback(
    (): CompletionDraftData => ({
      ...
      globalNotes,
      selfiePhotoIds: [],
      startWorkSelfiePhotoIds: startWorkSelfiePhotos.map((photo) => photo.id),
      startWorkReceiptPhotoIds: startWorkReceiptPhotos.map((photo) => photo.id),
      startWorkMaterialStores: startWorkMaterialStores.map((store) => ({
        id: store.id,
        name: store.name,
        city: store.city,
      ...
      itemStates: Object.fromEntries(
        [...itemStates.entries()].map(([itemId, state]) => [
          itemId,
          {
            afterPhotoIds: state.afterPhotos.map((photo) => photo.id),
  ...
  useEffect(() => {
    ...
    autosave
      ...
      .then((draft) => {
        ...
        const defaultStores = report.startMaterialStores.length > 0 
          ? report.startMaterialStores.map((store, idx) => ({
              id: `db-start-store-${idx}`,
              name: store.name,
              city: store.city,
              photos: (store.photoUrls || []).map((url, i) =>
                toRemotePhoto(url, i),
              ),
            }))
        ...
        for (const [itemId, draftState] of draft.itemStates) {
          ...
          mergedStates.set(itemId, {
            ...
            discountAmount:
              draftState.discountAmount ?? freshState.discountAmount,
          });
        }
  ...
  const handlePhotoCaptured = useCallback(
    async (file: File) => {
      ...
      if (cameraTarget.target === "startStore") {
        const storeId = cameraTarget.storeId;
        const photo = await autosave.addPhoto(reportNumber, file, "startStore");
        setStartWorkMaterialStores((prev) =>
          prev.map((s) =>
  ...
  const handleAddStartWorkMaterialStorePhoto = useCallback(
    (storeId: string, file: File) => {
      void autosave.addPhoto(reportNumber, file, "startStore").then((photo) => {
        setStartWorkMaterialStores((prev) =>
          prev.map((s) =>
            s.id === storeId ? { ...s, photos: [...s.photos, photo] } : s,
          ),
  ...
  const handleRemoveStartWorkStorePhoto = useCallback(
    ...
    if (shouldReviseStartWork) {
      if (startWorkSkipPhotos && !isZeroCost) {
        ...
        if (startWorkSelfiePhotos.length === 0) {
          errs.push({
            id: "start-work",
            message: "Foto selfie mulai pekerjaan wajib diisi",
          });
        }
        if (
          startWorkMaterialStores.length === 0 ||
          startWorkMaterialStores.some((store) => store.photos.length === 0)
        ) {
          errs.push({
            id: "start-work",
            message: "Setiap toko material mulai pekerjaan wajib melampirkan foto",
          });
        }
        if (startWorkReceiptPhotos.length === 0) {
          errs.push({
            id: "start-work",
            message: "Foto nota/struk mulai pekerjaan wajib diisi",
          });
        }
        if (
          startWorkMaterialStores.length === 0 ||
      ...
      const startWorkRevision: StartWorkRevisionInput | undefined =
        shouldReviseStartWork
          ? await (async () => {
              ...
              const finalizedStores = await Promise.all(
                startWorkMaterialStores.map(async (store, storeIndex) => {
                  ...
                  for (const photo of store.photos) {
                    ...
                    if (uploaded.urls[0]) urls.push(uploaded.urls[0]);
                    if (uploaded.fileIds[0]) uploadedStoreFileIds.push(uploaded.fileIds[0]);
                  }
                  return {
              ...
              if (allStorePhotosUploadFailed) return undefined;
