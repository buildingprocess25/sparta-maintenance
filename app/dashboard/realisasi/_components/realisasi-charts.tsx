...
import type { RealisasiMonthDatum, RealisasiBranchDatum } from "../queries";
...
const branchConfig = {
  ...
  totalRealisasi: { label: "Realisasi", color: "#ade4ea" },
};
...
export function RealisasiBranchChart({
  ...
  return (
    <ChartContainer config={branchConfig} className="w-full">
      <ComposedChart
        data={sliced}
        layout="vertical"
              ...
              labelFormatter={(_label, payload) => {
                ...
                return (
                  <div className="flex flex-col gap-0.5">
                    ...
                    <span className="text-xs text-muted-foreground">
                      {row?.completedCount ?? 0} laporan selesai
                    </span>
                  </div>
                );
              }}
