...
async function getDashboardPjumReportsInRange(params: { bmsNIK: string; branchNames: string[]; fromDate: Date; toDate: Date }): Promise<DashboardPjumCandidate[]> {
  ...
  return rows.map((row) => row as DashboardPjumCandidate).sort((left, right) => getPjumPeriodDate(left).getTime() - getPjumPeriodDate(right).getTime());
}

export async function getDashboardPjumBmsUsers(): Promise<DashboardPjumBmsUser[]> {
...
export async function searchDashboardPjumCandidates(input: { bmsNIK: string; from: string; to: string }): Promise<DashboardPjumCandidateResult> {
  const user = await requireRole("BMC");
  ...
  const hasBmsAccess = bmsUser?.role === "BMS" && !bmsUser.deletedAt && bmsUser.branchNames.some((branchName) => user.branchNames.includes(branchName));
  ...
  const activePjums =
    reportNumbers.length > 0
      ...
      : [];
  const numbersInActivePjum = new Set(activePjums.flatMap((pjum) => pjum.reportNumbers));
  ...
  const rows = reports.map((report): DashboardPjumCandidateRow => {
    const totalRealisasi = resolveReportTotalRealisasi(report.totalReal, report.items);
    const reportRequiresPjum = requiresPjum(report.totalReal, report.items);
    const isAlreadyInPjum = Boolean(report.pjumExportedAt) || numbersInActivePjum.has(report.reportNumber);
    const invalidReason = report.status !== "COMPLETED" ? "Belum selesai" : !reportRequiresPjum ? "Tidak perlu PJUM" : isAlreadyInPjum ? "Sudah masuk PJUM" : null;
...
export async function createDashboardPjum(input: { reportNumbers: string[]; bmsNIK: string; from: string; to: string; weekNumber: number }): Promise<{ error: string | null; pjumExportId: string | null }> {
  ...
  try {
    ...
    const bmsNIK = input.bmsNIK.trim();
    const safeNumbers = Array.from(new Set(input.reportNumbers.map((value) => value.trim()))).filter((value) => value.length > 0);
    ...
    const reportMap = new Map(reports.map((report) => [report.reportNumber, report]));

    const missingReport = safeNumbers.find((reportNumber) => !reportMap.has(reportNumber));
    if (missingReport) {
      return {
    ...
    const invalidReport = safeNumbers.map((reportNumber) => reportMap.get(reportNumber)).find((report) => !report || report.status !== "COMPLETED" || !requiresPjum(report.totalReal, report.items) || report.pjumExportedAt);
    if (invalidReport) {
      return {
        error:
          invalidReport.status !== "COMPLETED"
            ? `Laporan ${invalidReport.reportNumber} belum SELESAI`
            : !requiresPjum(invalidReport.totalReal, invalidReport.items)
    ...
    if (activeExportWithSelectedReports) {
      const duplicateNumbers = activeExportWithSelectedReports.reportNumbers.filter((reportNumber) => safeNumbers.includes(reportNumber));
      return {
        error: duplicateNumbers.length > 0 ? `Laporan ${duplicateNumbers.join(", ")} sudah masuk PJUM lain` : "Beberapa laporan sudah masuk PJUM lain",
        pjumExportId: null,
      };
    }

    const selectedReports = safeNumbers.map((reportNumber) => reportMap.get(reportNumber)).filter((report): report is DashboardPjumCandidate => !!report);
    ...
    const areaNames = [...new Set(selectedReports.map((report) => report.areaName).filter((areaName): areaName is string => Boolean(areaName)))];
    ...
    const pjumExport = await prisma.$transaction(async (tx) => {
      ...
      if (reportResult.count !== safeNumbers.length) {
        throw new Error("Data laporan berubah. Klik Cek Laporan lagi sebelum membuat PJUM");
      }
    ...
    const durationMs = Math.round(performance.now() - start);
    logger.error({ operation: "createDashboardPjum", correlationId, durationMs }, "Failed to create dashboard PJUM", error);
    return {
      error: error instanceof Error ? error.message : "Gagal membuat PJUM",
      pjumExportId: null,
    };
  }
}

export async function getAdminPjum(cursor: string | null, limit: number = 20, filters: AdminPjumFilters) {
  const correlationId = crypto.randomUUID();
  const start = performance.now();
  ...
  try {
    ...
    const scopedBranchNames = user.role === "ADMIN" ? null : user.role === "BMC" || user.role === "BNM_MANAGER" ? user.branchNames.filter((branchName) => branchName.trim() !== "") : undefined;
    ...
    const staleBefore = getStalePendingCutoff(pjumPolicy.pendingStaleDays);
    const baseWhere: Prisma.PjumExportWhereInput = scopedBranchNames === null ? { NOT: { branchName: EXCLUDED_ADMIN_BRANCH_NAME } } : { branchName: { in: scopedBranchNames } };
    ...
    if (filters.branchName && filters.branchName !== "all") {
      if (scopedBranchNames === null || scopedBranchNames.includes(filters.branchName)) {
    ...
    if (filters.areaName && filters.areaName !== "all") {
      if (scopedBranchNames === null || user.areaNames.length === 0 || user.areaNames.includes(filters.areaName)) {
    ...
    if (filters.fromDate || filters.toDate) {
      const { start, endExclusive } = getJakartaDateRange(filters.fromDate, filters.toDate);
    ...
    const durationMs = Math.round(performance.now() - start);
    logger.info({ operation: "getAdminPjum", correlationId, durationMs, count: pjumExports.length }, "Fetched admin PJUM successfully");
    ...
    const rows: PjumRow[] = pjumExports.map((p) => ({
      ...
      isStalePending: p.status === "PENDING_APPROVAL" && p.createdAt < staleBefore,
    }));
    ...
    const durationMs = Math.round(performance.now() - start);
    logger.error({ operation: "getAdminPjum", correlationId, durationMs }, "Failed to fetch admin PJUM", error);
...
export async function cancelAdminPjum(pjumExportId: string) {
  ...
  try {
    ...
    const deletedDriveFileIds = await deletePjumDriveFiles([pjumExport.pjumFinalDriveUrl]);
    ...
    const durationMs = Math.round(performance.now() - start);
    logger.error({ operation: "cancelAdminPjum", correlationId, durationMs, pjumExportId }, "Failed to cancel admin PJUM", error);
    return { error: "Gagal membatalkan PJUM" };
  }
}

async function deletePjumDriveFiles(urls: Array<string | null | undefined>): Promise<string[]> {
  const fileIds = [...new Set(urls.map(extractGoogleDriveFileId).filter((fileId): fileId is string => Boolean(fileId)))];
  ...
  for (const fileId of fileIds) {
    try {
      ...
      logger.error({ operation: "deletePjumDriveFiles", fileId }, "Failed to delete PJUM file from Drive", error);
...
function extractGoogleDriveFileId(value: string | null | undefined) {
  ...
  const fileViewMatch = normalized.match(/drive\.google\.com\/file\/d\/([^/?#]+)/);
