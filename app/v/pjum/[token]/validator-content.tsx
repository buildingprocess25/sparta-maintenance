...
const statusView = {
    VALID: {
        ...
        description:
            "Data ini berasal dari PJUM yang sudah disetujui di SPARTA.",
        icon: CheckCircle2,
        badgeClass:
    ...
    NEEDS_REVIEW: {
        ...
        description:
            "PJUM ditemukan, tetapi metadata persetujuan atau PDF final belum lengkap.",
        icon: AlertTriangle,
        badgeClass:
    ...
    INVALID: {
        ...
        description:
            "PJUM ditemukan, tetapi statusnya tidak memenuhi syarat dokumen final.",
        icon: XCircle,
        badgeClass: "",
        alertClass: "",
...
export function ValidatorContent({
    ...
    if (result.kind === "not-found") {
        return (
            <main className="min-h-dvh bg-muted/30 px-4 py-4 text-foreground sm:px-6 sm:py-6">
                <div className="mx-auto max-w-3xl">
                    <Alert variant="destructive">
                        <XCircle aria-hidden="true" />
                        <AlertTitle>Dokumen PJUM tidak dapat diverifikasi</AlertTitle>
                        <AlertDescription>
                            Kode QR tidak terdaftar di SPARTA atau format link
            ...
            <div className="mx-auto grid max-w-5xl gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
                ...
                <Card size="sm" className="lg:row-span-2">
                    <CardHeader>
                        ...
                        <CardDescription>
                            Cocokkan dengan kode yang tercetak di PDF.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div className="flex flex-col gap-3">
                            <div className="break-all rounded-md border bg-muted/30 px-3 py-2 font-mono text-sm font-semibold">
                                {result.displayCode}
                            </div>
                            <Badge
                                variant={
                ...
                <Card size="sm">
                    <CardHeader>
                        ...
                        <CardDescription>
                            Gunakan data ini untuk mencocokkan dokumen cetak.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        <dl className="grid gap-2 sm:grid-cols-2">
                            ...
                            <Meta
                                label="BMS"
                                value={`${result.bmsName} (${result.bmsNIK})`}
                            />
                            <Meta
                                label="Periode"
                                value={`${formatDate(result.fromDate)} - ${formatDate(result.toDate)}`}
                            />
                            <Meta
                                label="Minggu/Bulan"
                                value={`Minggu ke-${result.weekNumber}${
                                    result.monthName
                                        ? `, ${result.monthName}`
                                        : ""
                                }`}
                            ...
                            <Meta
                                ...
                                value={formatCurrency(result.totalExpenditure)}
                            />
                            <Meta
                                label="Jumlah Laporan"
                                value={`${result.reportCount} laporan`}
                            />
                            <Meta
                                ...
                                value={
                                    result.approverName
                                        ? result.approverNIK
                                            ? `${result.approverName} (${result.approverNIK})`
                                            : result.approverName
                                        : "-"
                                }
                            />
                            <Meta
                                ...
                                value={formatDate(result.approvedAt)}
                            />
                        </dl>
