...
async function main() {
    ...
    if (newStores.length === 0) {
        console.log(`Sinkronisasi selesai: ${stores.length} baris, 0 toko baru.`);
        return;
    }
    ...
    console.log(
        `Sinkronisasi selesai: ${stores.length} baris, ${result.count} toko baru, ${stores.length - result.count} dilewati.`,
    );
}
