...
for pattern in "${legacy_patterns[@]}"; do
  if grep -Erq "$pattern" "$AGENT_DIR/skills"; then
...
for pattern in "${mapping_checks[@]}"; do
  if grep -Erq "$pattern" "$AGENT_DIR/AGENTS.md"; then
