...
export function BmsBalanceCard({
    balance,
    className,
    compact = false,
}: BmsBalanceCardProps) {
    ...
    const usagePercent =
        balance.initialBalance > 0
            ? Math.min(100, Math.round((usedAmount / balance.initialBalance) * 100))
            : 0;
        ...
        return (
            ...
            >
                ...
                <span className="font-medium">
                    {isLocked
                        ? "Saldo Terkunci (PJUM)"
                        : `Sisa Saldo: ${formatCurrency(balance.availableBalance)}`}
                </span>
            </div>
        );
    }
        ...
        >
            ...
            {isLocked && (
                <p className="mt-2 text-xs leading-relaxed text-red-600 dark:text-red-400">
                    Saldo Anda sedang terkunci karena ada PJUM yang menunggu persetujuan BNM Manager.
                    Anda tidak dapat memulai pekerjaan baru hingga PJUM diproses.
                </p>
            )}
